...
function DiagramBeam({ position, args, rotation = [0, 0, 0], color }: DiagramBeamProps) {
  return (
...
function RoofDiagram({ highlight }: { highlight: string }) {
  const ridgeY = 1.5;
  const eavesY = 0;
  const eavesX = 2.7;
  const roofHalfZ = 2.2;
  ...
  const c = (type: string) => type === highlight ? BEAM_TYPES[highlight]?.color || '#fff' : GHOST;
...
export default function BeamDetailPage() {
  ...
  const beam = type ? BEAM_TYPES[type] : undefined;
